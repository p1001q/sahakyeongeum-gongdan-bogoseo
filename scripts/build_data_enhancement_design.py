...
def field_rows() -> list[dict[str, str]]:
    ...
    add(service, 10, "phone_consult_transfer_count", "전화상담전환건수", "측정값", "정수", "0 이상", "필수", "3단계", "디지털→전화 전환율", "상담전환건수의 채널 부문; 내부 집계키만 사용")
    add(service, 11, "online_consult_transfer_count", "온라인상담전환건수", "측정값", "정수", "0 이상", "필수", "3단계", "전환채널 구성", "상담전환건수의 채널 부문; 내부 집계키만 사용")
    add(service, 12, "visit_consult_transfer_count", "방문상담전환건수", "측정값", "정수", "0 이상", "필수", "3단계", "전환채널 구성", "상담전환건수의 채널 부문; 내부 집계키만 사용")
    add(service, 13, "other_consult_transfer_count", "기타상담전환건수", "측정값", "정수", "0 이상", "필수", "3단계", "전환채널 구성", "상담전환건수의 채널 부문; 미분류 포함기준 명시")
    add(service, 14, "repeat_consult_count", "반복상담건수", "측정값", "정수", "0 이상", "권장", "3단계", "반복문의 관리", "반복 기준기간 명시; 공개키 금지")
    add(service, 15, "suppression_status", "소수셀처리상태", "품질", "범주", "공개|억제|상위범주통합", "필수", "3단계", "재식별 위험 관리", "기준 미만 셀·보완억제 표시")
    add(service, 16, "aggregation_version", "집계기준버전", "품질", "문자열", "예: v1.0 형식", "필수", "3단계", "산식 변경 추적", "수정이력 연결")
...
def kpi_rows() -> list[dict[str, str]]:
    return [
        ...
        {"KPI": "디지털→전화 전환율", "산식": "전화상담전환건수÷업무시작건수×100", "필요데이터": "업무처리현황", "현재산출": "불가", "주의": "전체 상담전환 중 전화 채널 부문; 채널 배정 규칙 명시"},
...
def main() -> None:
    ...
    required_checks = {
        ...
        "상담전환_채널부문": fields["필드명"].isin(["phone_consult_transfer_count", "online_consult_transfer_count", "visit_consult_transfer_count", "other_consult_transfer_count"]).sum() == 4,
